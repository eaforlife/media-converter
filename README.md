...
Streaming and Cellular retain `video_bitrate=0`, so their capped quality-based VBR encoders can reduce bitrate on easy scenes instead of targeting a fixed average. When a source exceeds 24 fps, these presets output 23.976 fps without retiming the audio; encoded audio retains timestamp-aware asynchronous resampling for gap and overlap protection.
