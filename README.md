...
Version 0.3.2 is the current stable release. Only video input is supported, and a compatible hardware video encoder is required. There is no software-encoding fallback.
...
```bash
sudo apt install ./ea-media-tools_0.3.2_amd64.deb
```
...
Windows x64 builds check the public Squirrel feed at startup and once per hour. **Versions 0.3.0 and 0.3.1 use the repository's former URL and must be upgraded to v0.3.2 manually once.** Later Windows x64 releases can then be discovered through **Check for update**. Windows ARM64 and unsigned macOS builds should download new releases manually; Linux updates are provided through the installed package manager.
...
ARM64 describes the application and Jellyfin FFmpeg runtime architecture, not guaranteed GPU support. Qualcomm/Adreno on Windows and Mali/Rockchip on Linux are not supported in 0.3.2; those systems can encode only if one of the NVIDIA, Intel, or AMD backends below passes detection. Apple silicon uses VideoToolbox and is supported on macOS.
