...
Version 2.6.3, codename **jaguar**, is the current stable release. It supports video files, short music videos with attached artwork, and recursive audio libraries. Hardware acceleration is enabled by default but can be disabled for CPU video encoding and decoding.
...
| --- | --- | --- |
| Windows | `EA-Media-Tools-2.6.3-x64-Setup.exe` | `EA-Media-Tools-2.6.3-arm64-Setup.exe` |
...
```bash
sudo apt install ./ea-media-tools_2.6.3_amd64.deb
```
...
Streaming selects HEVC Main for SDR output at 4K and 1080p, and defaults to HEVC Main10 for HEVC Main10, HDR, or Dolby Vision sources when the selected encoder passed its 10-bit capability test. The independent Main10 switch remains available for every capable HEVC encoder, so that automatic choice can be overridden for any preset or SDR source. NVIDIA SDR conversion uses CUDA's native `p010le` output option inside `scale_cuda`, keeping the decoded frames on the GPU through NVENC. Auto Scale and explicit scaling switch Streaming to AV1 at 720p and 360p. H.264 exposes a Filters-tab High-profile toggle that defaults on whenever H.264 is selected. Archive and Regular select H.264 by default. At 1080p, the H.264 maximum rates are 10000 kbps for Archive, 8000 kbps for Regular, 6500 kbps for Streaming, and 7000 kbps for Music Video. At 720p and 360p/Cellular, the H.264 maximum rate is 4000 kbps. Regular and Streaming use P4 for H.264 at 4K and 1080p, Music Video uses P6, and all three use P2 at 720p and 360p; Archive retains P6.
